...
#define total_dis_size 70000000
...
// At the beggining, delete_smallest has root as dir and the size of root as current_min
// Then it goes trough every directory and checks for sizes lower then current_min
// therefore returning the smallest size possible

int delete_smallest (struct Directory * dir, int size_min, int current_min) {
    if (dir->size > size_min && dir->size < current_min) {
        current_min = dir->size;
    }
    for (int i = 0; i < dir->num_subdirs; i++) {
        current_min = delete_smallest (dir->subdirs[i],size_min,current_min);
    }
  return current_min;
}
...
int main () {
    ...
    // Minimum size needed for the update
    int min_size = 30000000 - (70000000 - root->size);
    int answer = delete_smallest (root,min_size,root->size);
    printf ("Answer: %d\n",answer);
    fclose (input);
}